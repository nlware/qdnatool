	...
	<div class="modal-header">
		...
		echo $this->Form->postLink(
			'×', array(
				...
				'class' => 'close',
				'div' => false,
				'confirm' => __('Are you sure you want to close this wizard?')
			)
		);
		?>
	...
	<div class="modal-body" style="height:400px;">
		<ul class="nav nav-tabs">
			<li<?php echo ($offset === 'overview'?' class="active"':''); ?>>
				<?php
				echo $this->Html->link(__('Exam'), array('action' => 'stevie', $exam['Exam']['id']));
				?>
			</li>
			<?php
			if (!empty($exam['Item'])):
				...
				foreach ($exam['Item'] as $i => $item):
					if (!empty($item['Messages'])):
						...
						<li<?php echo (($offset == ($i + 1))?' class="active"':''); ?>>
							<?php
							echo $this->Html->link(__('Item %s', ($i + 1)), array('action' => 'stevie', $exam['Exam']['id'], ($i + 1)));
							?>
						</li>
						<?php
			...
			?>
					<li<?php echo (!empty($offset) && ($offset === 'conclusion')?' class="active"':''); ?>>
						<?php
						echo $this->Html->link(__('Conclusion'), array('action' => 'stevie', $exam['Exam']['id'], 'conclusion'));
						?>
					</li>
		...
		if ($offset == 'conclusion'):
			...
			if ($exam['Exam']['cronbachs_alpha'] < 0.7):
				...
				<p>
					<?php
					echo __(
						'The reliability of the test is too low to give a decisive result: there are too many incorrect fail ' .
						'or pass decisions being made.'
					);
					?>
				</p>
				<p>
					<?php
					echo __('The reliability of the test can be increased by executing an item analysis and removing or ' .
					'editing incorrect items from the test results.');
					?>
				</p>
				<p>
					<?php
					echo __(
						'Press on ‘next’ to start the item analysis. Only items which can increase reliability will be shown. ' .
						'Reliable questions will not be shown.'
					);
					?>
				</p>
				<?php
			elseif ($exam['Exam']['cronbachs_alpha'] < 0.8):
				?>
				<h6><?php echo __('Reliability of the exam: doubtful'); ?></h6>
				<p>
					<?php
					echo __(
						'The reliability of this test is debatable. An item analysis should be executed to increase the reliability ' .
						'of the test: there are too many incorrect fail or pass decisions being made.In case this is a mid term ' .
						'test, the reliability is sufficient.'
					);
					?>
				</p>
				<p>
