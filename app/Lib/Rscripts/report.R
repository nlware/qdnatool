#file_path = choose.dir() #File path where the pdf should be saved
#file_name="Meerdere juist antwoorden.pdf" # Fill in file name where the pdf should be saved
#title = paste("Put the title here")
#
#make_pdf(file_name,file_path,number_students,number_answeroptions,number_questions,Cronbach,
#         frequency_answer_options, percentage_answer_options, input_correct, key,
#         correct_frequency,correct_percentage, corrected_item_tot_cor,
#         corrected_item_tot_cor_answ_option,title,item_names)
#

make_pdf=function(file_name,number_students,number_answeroptions,number_questions,Cronbach,
                  frequency_answer_options, percentage_answer_options, input_correct, key,
                  correct_frequency,correct_percentage, corrected_item_tot_cor,
                  corrected_item_tot_cor_answ_option,title,item_names)
{  
#  setwd(file_path)
  ...
  {Correct = rep("Incorrect",number_answeroptions[i] + 1)
   ...
   item_list[[i]] = data.frame(c(LETTERS[1:number_answeroptions[i]],"Missing"),
                               c(frequency_answer_options[c(2:(number_answeroptions[i]+1),1),i]), #Frequency is also stored at this point, but not used.
  ...
  {
    item_list1[[i]]$Ans_Factor = factor(item_list1[[i]]$"Answer Option", levels = c(LETTERS[1 : max(number_answeroptions)] , "Missing"))
  ...
  if(any(key!=0))
  {
    AnsOpt_dataframe = ldply(item_list1[number_answeroptions != 0 & number_answeroptions < 15], data.frame)
    ...
    AnsOpt_dataframe$Ans_Factor = factor(AnsOpt_dataframe$Ans_Factor,  levels = c(LETTERS[1 : max(number_answeroptions)] , "Mis"))
    AnsOpt_dataframe$id = factor(AnsOpt_dataframe$id , levels = items[number_answeroptions!=0])
  ...
  dataframe_correct = data.frame(factor(1:number_questions),correct_frequency,correct_percentage,corrected_item_tot_cor)
  ...
  pdf(file=file_name,h=8,w=10)
  
  #Textplot plots outside the normal plot window, therefore xpd=NA
  par(xpd=NA,
      mar = rep(2, 4))
  
  if(max(number_answeroptions) < 8) #If maximum 7 answer options 8 items per page
  {  
    ...
    {textplot(item_list[[i]][,1:4],show.rownames=FALSE,cex=1,valign="top",mar=c(5,1,0,1))
     ...
     {for(j in 1 : sum(key[,i] == 1 ))          
       rect(.19,.95 - .065 * which(key[,i] == 1)[j] , .85, 1.01 - .065*which(key[,i] == 1)[j],
            col=rgb(0,.9,0,.5),density=NA)
    }
    }
