 ...
 */
	...
	public function setup(Model $Model, $config = array()) {
		...
		$this->settings[$Model->alias] = array_merge($this->__defaults, $config);
 ...
 */
	public function movedown(Model $Model, $id = null, $number = 1) {
		...
		$oldWeight = $this->__read($Model, $id);
		if (empty($Model->data)) {
			return false;
		}
		if (is_numeric($number)) {
			if ($number == 1) { // move node 1 space down
				$next = $this->__next($Model);
				...
				if (!$this->__findbyweight($Model, $newWeight)) {
					return false;
				}
				...
				$Model->set($this->settings[$Model->alias]['field'], $newWeight);
	...
	public function moveup(Model $Model, $id = null, $number = 1) {
		...
		if (is_numeric($number)) {
			if ($number == 1) { // move 1 space
				$previous = $this->__previous($Model);
				if (!$previous) {
					return false;
				}
 ...
 */
	...
	private function __newWeight($Model, $foreignKey = null) {
		...
		$highest = $this->__highest($Model, $foreignKey);
